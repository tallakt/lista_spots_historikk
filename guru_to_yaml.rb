...
(2006..2012).each do |year|
  ...
  doc.css('tr').each do |tr|
    ...
    if cells.size == 41 && cells.first.match(/\d\d\.\d\d\.\d\d\d\d/)
      date = cells.shift
      ...
      data = Hash[[2, 5, 18, 11, 14, 17, 20, 23].zip(wind_speeds.zip(wind_dirs, wave_heights, wave_dirs, temperatures).map do |dd| 
        Hash[[:wind, :wind_dir, :wave, :wave_dir, :t].zip dd]
      end)]
